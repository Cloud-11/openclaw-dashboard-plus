...
function readJsonDirectory(sourceDir) {
  ...
  return Object.fromEntries(
    fs
      ...
      .map(entry => [path.basename(entry.name, ".json"), readJson(path.join(sourceDir, entry.name))]),
  );
}
...
function hydrateStyleBundle(styleBundlePath) {
  const styleBundle = fs.existsSync(styleBundlePath) ? readJson(styleBundlePath) : { schemaVersion: 1, version: "builtin", modules: [] };
  ...
  return {
    ...
    modules: modules.map(module => {
      ...
      const assetPath = typeof module.assetPath === "string" && module.assetPath.trim() ? module.assetPath.trim() : "";
...
function writeExtensionFiles() {
  ...
  const extensionVersion =
    typeof pluginMetadata.version === "string" && pluginMetadata.version.trim() ? pluginMetadata.version.trim() : manifest.version;
  const themePresets = readJson(themePresetsPath);
  const uiStylePresets = readJson(uiStylePresetsPath);
  const styleBundle = fs.existsSync(extensionStyleBundlePath) ? readJson(extensionStyleBundlePath) : { version: `builtin-${extensionVersion}` };
  ...
  const contentSource = createExtensionContentSource(readText(extensionContentSourcePath), themePresets, uiStylePresets, hydratedStyleBundle);
  ...
  fs.writeFileSync(contentPath, `// This file is generated by openclaw-dashboard-plus/build-extension.mjs.\n${contentSource}\n`, "utf8");
